...
class Employees(Base):
    ...
    def post(self, request):
        ...
        enterprise_id = self.get_enterprise_id(request.user.id)
        singup_user = Authentication.signup(
            self,
            name=name,
            email=email,
            password=password,
        ...
        if isinstance(singup_user, User):
            return Response({"success": True}, status=status.HTTP_201_CREATED)
        
        return Response(singup_user, status=status.HTTP_400_BAD_REQUEST)
...
class EmployeeDetail(Base):
    ...
    def put(self, request, employee_id):
        ...
        if email != employee.user.email and User.objects.filter(email=email).exists():
            raise APIException("Este email já está em uso", code="email_already_use")
    ...
    def delete(self, request, employee_id):
        ...
        check_is_owner = User.objects.filter(id=employee.user.id, is_owner=1).exists()

        if check_is_owner:
            return APIException("Você não pode demitir o dono da empresa")
